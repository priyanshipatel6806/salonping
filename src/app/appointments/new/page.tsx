...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!salon) return
      setSalonId(salon.id)
      const { data: svcs } = await supabase.from('services').select('id,name,price,duration_minutes')
        .eq('salon_id', salon.id).eq('active', true).order('name')
      setServices(svcs || [])
    }
    load()
  }, [])
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!form.client_name || !form.client_phone || !form.service || !form.date || !form.time) {
    ...
    const { data: existing } = await supabase.from('appointments').select('id')
      .eq('salon_id', salonId).eq('scheduled_at', scheduled_at).eq('status', 'confirmed').single()
    if (existing) {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    borderRadius: 10, padding: '12px 14px', fontSize: 14, color: '#fff', outline: 'none', boxSizing: 'border-box',
  }
    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh', color: '#fff' }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
              ...
              <div>
                ...
                <input type="tel" value={form.client_phone} onChange={e => setForm({ ...form, client_phone: e.target.value })}
                  placeholder="+1 226 555 0123" required style={inputStyle} />
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24, marginBottom: 14 }}>
            ...
            <select value={form.service} onChange={e => setForm({ ...form, service: e.target.value })}
              ...
              {services.length > 0
                ...
                : <option disabled>No active services — add services first</option>
              }
            </select>
            {services.length === 0 && (
              <p style={{ fontSize: 12, color: '#f87171', marginTop: 8 }}>
                ⚠️ No services found. <a href="/services" style={{ color: GOLD }}>Add your services first →</a>
              </p>
            )}
            {selectedService && (
          ...
          <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24, marginBottom: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
              {[
                ...
                { value: 'whatsapp', label: '💬 WhatsApp', desc: 'WhatsApp' },
                ...
                <button key={opt.value} type="button" onClick={() => setForm({ ...form, reminder_channel: opt.value })}
                  style={{
                    padding: '12px', borderRadius: 10, textAlign: 'center', cursor: 'pointer', border: 'none',
          ...
          {error && (
            <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 10, padding: '12px 16px', marginBottom: 14, fontSize: 13, color: '#f87171' }}>
              {error}
            </div>
          )}

          <div style={{ display: 'flex', gap: 12 }}>
            <a href="/appointments" style={{ flex: 1, padding: '13px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.5)', fontSize: 14, fontWeight: 600, textDecoration: 'none', textAlign: 'center', display: 'block' }}>
              Cancel
            </a>
            <button type="submit" disabled={loading}
              style={{ flex: 2, background: `linear-gradient(135deg,#2a1f08,${GOLD})`, color: '#0a0a0a', fontWeight: 800, fontSize: 14, padding: '13px', borderRadius: 12, border: 'none', cursor: 'pointer', opacity: loading ? 0.7 : 1 }}>
