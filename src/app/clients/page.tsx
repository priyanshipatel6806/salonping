...
export default async function ClientsPage() {
  ...
  const { data: services } = await supabase.from('services').select('name,price')
              ...
              <div key={c.phone} style={{display:'grid', gridTemplateColumns:'2fr 1.5fr 1fr 1fr 1fr 1fr 1fr', gap:0, padding:'16px 24px',
                ...
                <div style={{display:'flex', alignItems:'center', gap:12}}>
                  <div style={{width:36, height:36, borderRadius:'50%', background:`linear-gradient(135deg,#2a1f08,${GOLD})`,
                    display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:13, color:'#0a0a0a', flexShrink:0}}>
                ...
                <div style={{display:'flex', alignItems:'center'}}>
                  ...
                  {c.visits >= 5 && <span style={{marginLeft:6, fontSize:10, fontWeight:700, padding:'2px 6px', borderRadius:100,
                    background:'rgba(201,168,76,0.15)', color:GOLD}}>VIP</span>}
                </div>
                {/* Spend */}
                <div style={{display:'flex', alignItems:'center'}}>
